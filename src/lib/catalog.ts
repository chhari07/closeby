 ...
 * duplicate check. Without it each of those re-read every product row on
 * every view/search.
 *
 ...
 * scripts/ folder, the Supabase dashboard). Order placement and the AI cart's
 * draftCart still read the individual product rows live, so a slightly old
...
export function getShopCatalog(shopId: string): Promise<ProductDoc[]> {
  return unstable_cache(
    async () => {
      const rows = await db()`select * from products where shop_id = ${shopId} order by updated_at desc`;
